...
class CSV_Writer(object):
    ...
    def from_path(cls, path_to_tweet_file):
        ...
        with open(path_to_tweet_file, "a", encoding="utf-8", newline="") as tobetweeted:
    ...
    def cut_context(self, bor: Borrowing) -> str:
        selected_context_tokens = (
            bor.context_tokens[0 : bor.end_pos + 15]
            if bor.start_pos < 15
            else bor.context_tokens[bor.start_pos - 15 : bor.end_pos + 15]
        )
        selected_context_text = " ".join(
            [token.text for token in selected_context_tokens]
        )
